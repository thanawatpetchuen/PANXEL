...
class MyMplCanvas(FigureCanvas):
    ...
    def __init__(self, parent=None, width=5, height=4, dpi=100):
        ...
        # axpos = plt.axes([0.2, 0.1, 0.55, 0.03], facecolor=axcolor)
        #
        # self.spos = Slider(axpos, 'Pos', 0.1, 90.0)
    ...
    # def update(self):
    #     # pos = self.spos.val
    #     myAxes = self.axes.axis()
    #     myXmin = myAxes[0]
    #     myXmax = myAxes[1]
    #     myYmin = myAxes[2]
    #     myYmax = myAxes[3]
    #     # myAxes3 = myAxes[3]
    #     self.axes.axis([pos, pos + 10, myYmin, myYmax])
    #     print(self.axes.axis())
...
"border-top-color: rgb(0, 0, 0);")
    ...
    def clickOnMe(self):
        ...
        if not self.time_count:
            ...
            data_ss = self.data[dim+meas]
            for item in dim:
                data_ss = data_ss.loc[data_ss[item] == self.dim_dict['combobox>{}'.format(item)]]
            # data_cat = data_sss.loc[data_sss['Category'].isin(['Technology'])]
            self.sc.update_figure(data_ss)
